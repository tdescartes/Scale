...
export default function ControlPanel({ onAlgorithmChange, onFailureInjection }: ControlPanelProps) {
  ...
  const { loading: isProcessing, error, success, execute, setSuccess, setError } = useApiRequest();
  ...
  const handleInjectFailure = async () => {
    ...
    const response = await execute('/api/failure/inject', {
      method: 'POST',
  ...
  const handleChangeAlgorithm = async () => {
    const response = await execute('/api/algorithm/change', {
      method: 'POST',
    ...
    if (response?.message) {
      ...
      setSuccess(`Algorithm changed to ${algorithm}`);
    }
  ...
  const handleToggleAutoTuning = async () => {
    ...
    const response = await execute('/api/autoscaling/toggle', {
      method: 'POST',
      body: JSON.stringify({ enabled: newState }),
    });

    if (response) {
      if (newState && response.config) {
        setSuccess(`Dynamic pod scaling enabled - ${response.config.min_pods}-${response.config.max_pods} pods`);
      } else {
        setSuccess('Dynamic pod scaling disabled - pod count will remain fixed');
      }
    } else {
      setAutoTuning(!newState); // Revert on error
    }
  };
